...
void plotPostNuisance_combine(char* mlfit="mlfit.root", double MaxUncertainty = 1.0, 
  ...
  for(int i=0; i<parlist.getSize(); i++) {
    ...
    TString name_tstr = parlist[i].GetName(); 
		...
		if (name_tstr.Contains("stat")) continue;
  ...
  for(int i=0; i<nbins-2; i++) {
    ...
    h1_ref->GetXaxis()->SetBinLabel(i+2, nuisance[i].ReplaceAll("model_mu_cat_monotop","mu").ReplaceAll("monotop_",""));
  ...
  h1_ref->SetMinimum(-10.); 
  h1_ref->SetMaximum(10.); 
